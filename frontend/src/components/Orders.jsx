...
import { faTable, faTimes, faPenFancy, faShoppingBag, faTrash, faBoxes, faCheck, faExclamationTriangle, faMoneyBill1Wave } from '@fortawesome/free-solid-svg-icons';
...
export const Orders = () => {
    ...
    const handleSaveSale = () => {
        ...
        axios.post('http://127.0.0.1:8000/api/sale', formData, {
            headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/json',
            },
    ...
    return (
        <div className='relative'>
            <div className={`${showSale ? 'block' : 'hidden'}  fixed w-full h-full form z-50 flex items-center justify-center`}>
                <button onClick={toggleSale} className='absolute top-3 right-8 text-7xl text-white'>
                ...
                <div className='w-auto h-auto bg-white rounded-md px-10'>
                    ...
                    <h3 className='my-5 text-center text-3xl'>Precio total: <span className='text-amber-600'>{totalPrice}</span><input className='hidden w-28 text-amber-600 focues:border-0' type="number" name="" id="" value={totalPrice} onChange={handleChange} readOnly/></h3>
                    <button onClick={handleSaveSale} className='block m-auto my-5 bg-green-700 hover:bg-green-400 rounded-md duration-300 px-5 py-2'>Confirmar <FontAwesomeIcon icon={faMoneyBill1Wave} className='ml-2' /></button>
                </div>
            </div>
            <Header />
            ...
            <form className='relative w-11/12 h-96 m-auto mt-12 mb-28' action="">
                <div className='w-full flex items-center justify-between'>
                    <div>
                        <button className='bg-green-700 hover:bg-green-400 rounded-md duration-300 px-5 py-2' onClick={toggleSale}>Cobrar <FontAwesomeIcon icon={faMoneyBill1Wave} className='ml-2' /></button>
